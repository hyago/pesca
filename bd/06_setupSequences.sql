...
INNER JOIN (
    ...
    FROM ACCESS.ENTREV_PESCA AS EP WHERE EP.TP_REDE='2' ORDER BY EP.COD_FICHADIARIA, EP.CODIGO
...
WHERE EP.ARTE = '1' ORDER BY EP.CODIGO ,  FD.COD_FICHA );

---> Não Monitoradas
INSERT INTO T_MONITORAMENTO (MNT_ID,  MNT_ARTE, MNT_QUANTIDADE, MNT_MONITORADO,  FD_ID)
SELECT NEXTVAL('T_MONITORAMENTO_MNT_ID_SEQ'), CAST(EP.ARTE AS INT4),  FD.ARRFUNDONM,  TRUE, FD.COD_FICHA
FROM ACCESS.FICHA_DIARIA AS FD
INNER JOIN (
    SELECT DISTINCT ON (EP.COD_FICHADIARIA) COD_FICHADIARIA,  EP.ARTE, EP.CODIGO 
    FROM ACCESS.ENTREV_PESCA AS EP WHERE EP.ARTE='1' ORDER BY EP.COD_FICHADIARIA
